  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        <button onClick={() => handleFilterChange("all")} className={`px-3 py-1.5 rounded-lg text-sm transition ${filter === "all" ? "bg-[#CAB276] text-black" : "bg-gray-800 text-gray-400 hover:bg-gray-700"}`}>
          All ({statusCounts.all})
        </button>
        {statusOptions.map((status) => (
          <button key={status} onClick={() => handleFilterChange(status)} className={`px-3 py-1.5 rounded-lg text-sm capitalize transition ${filter === status ? "bg-[#CAB276] text-black" : "bg-gray-800 text-gray-400 hover:bg-gray-700"}`}>
          ...
          <div className="space-y-4">
            {paginatedOrders.map((order) => (
              <div key={order.id} className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
                <div className="p-4 flex items-center gap-4 cursor-pointer hover:bg-gray-800/30 transition" onClick={() => setExpandedId(expandedId === order.id ? null : order.id)}>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <p className="text-white font-medium">{order.customerName || order.customerEmail || "Customer"}</p>
                      <span className={`text-xs px-2 py-0.5 rounded capitalize ${statusColors[order.status] || statusColors.pending}`}>{order.status}</span>
                    </div>
                  ...
                  {expandedId === order.id ? <ChevronUp className="w-5 h-5 text-gray-500" /> : <ChevronDown className="w-5 h-5 text-gray-500" />}
                </div>
                  ...
                  <div className="border-t border-gray-800 p-4 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-white">{order.customerPhone || order.customerEmail || "N/A"}</p>
                      </div>
                      <div>
                        <h4 className="text-sm text-gray-500 mb-1">Email</h4>
                        <p className="text-white">{order.customerEmail || "N/A"}</p>
                      </div>
                    </div>
                      ...
                      <div className="space-y-2">
                        {order.items?.map((item) => (
                          <div key={item.id} className="flex items-center gap-3 bg-gray-800/50 rounded-lg p-2">
                            ...
                            <p className="text-[#CAB276] text-sm">{formatPrice(item.lineTotal)}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {statusOptions.map((status) => (
                          <button key={status} onClick={() => updateOrderStatus(order.id, status)} disabled={order.status === status}
                            className={`px-3 py-1 rounded text-xs capitalize transition ${order.status === status ? "bg-gray-700 text-gray-500 cursor-not-allowed" : "bg-gray-800 text-gray-300 hover:bg-gray-700"}`}>
                            {status}
                          </button>
                        ))}
                      </div>
