...
export default function AccountPage() {
  ...
  const inputClasses = "w-full border-2 border-gray-800 bg-black text-white rounded-xl px-3 md:px-4 py-2.5 md:py-3 focus:outline-none focus:border-[#CAB276] focus:ring-4 focus:ring-[#CAB276]/10 transition-all placeholder:text-gray-500 disabled:bg-gray-900 disabled:text-gray-500 text-sm md:text-base";
    ...
    <div className="min-h-screen bg-black">
      ...
      <header className="bg-black py-3 px-4 border-b border-gray-800 sticky top-0 z-40">
        <div className="container mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <Link href="/account" className="p-2 bg-gray-900 rounded-full">
              <User className="w-5 h-5 text-[#CAB276]" />
            </Link>
          </div>
        </div>
        {mobileMenuOpen && (
      ...
      <div className="container mx-auto px-4 py-6 md:py-10">
        ...
        <div className="md:hidden mb-4">
          <button onClick={() => setMobileSidebarOpen(!mobileSidebarOpen)} className="w-full flex items-center justify-between p-3 bg-gray-900 rounded-xl shadow-sm border border-gray-800 text-white">
            <span className="font-medium">{tabs.find(t => t.id === activeTab)?.label}</span>
            <span className="text-gray-400">▼</span>
          </button>
          {mobileSidebarOpen && (
            <div className="mt-2 bg-gray-900 rounded-xl shadow-sm overflow-hidden border border-gray-800">
              {tabs.map(tab => {
                const Icon = tab.icon;
                return (
                  <button key={tab.id} onClick={() => { setActiveTab(tab.id); setMobileSidebarOpen(false); }}
                    className={`w-full flex items-center gap-3 px-4 py-3 text-left transition ${activeTab === tab.id ? 'bg-[#CAB276] text-white' : 'text-gray-400 hover:bg-gray-900'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-8">
          ...
          <div className="hidden md:block md:col-span-1">
            <div className="bg-gray-900 rounded-2xl shadow-sm p-6 sticky top-24 border border-gray-800">
              <div className="text-center mb-6 pb-6 border-b border-gray-800">
                <div className="relative w-20 h-20 mx-auto mb-3">
                  ...
                  <button className="absolute bottom-0 right-0 w-7 h-7 bg-[#CAB276] rounded-full flex items-center justify-center shadow-lg">
                    <Camera className="w-3.5 h-3.5 text-white" />
                  </button>
                </div>
                <h3 className="font-semibold text-white">{profile.firstName} {profile.lastName}</h3>
                ...
                {tabs.map(tab => {
                  ...
                  return (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                      className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition ${activeTab === tab.id ? 'bg-[#CAB276] text-white' : 'text-gray-400 hover:bg-gray-900'}`}>
                      <Icon className="w-5 h-5" /><span className="font-medium">{tab.label}</span>
                    </button>
                  );
                })}
              </nav>
              <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left text-red-500 hover:bg-red-900/30 transition mt-4">
            ...
            {activeTab === "profile" && (
              <div className="bg-gray-900 rounded-xl md:rounded-2xl shadow-sm p-5 md:p-8 border border-gray-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                  <div><label className="block text-xs md:text-sm font-medium text-gray-300 mb-1.5 md:mb-2">First Name</label><input type="text" value={profile.firstName} onChange={(e) => setProfile({...profile, firstName: e.target.value})} disabled={!isEditing} className={inputClasses} /></div>
                  <div><label className="block text-xs md:text-sm font-medium text-gray-300 mb-1.5 md:mb-2">Last Name</label><input type="text" value={profile.lastName} onChange={(e) => setProfile({...profile, lastName: e.target.value})} disabled={!isEditing} className={inputClasses} /></div>
                  <div><label className="block text-xs md:text-sm font-medium text-gray-300 mb-1.5 md:mb-2">Email</label><input type="email" value={profile.email} onChange={(e) => setProfile({...profile, email: e.target.value})} disabled={!isEditing} className={inputClasses} /></div>
                  <div><label className="block text-xs md:text-sm font-medium text-gray-300 mb-1.5 md:mb-2">Phone</label><input type="tel" value={profile.phone} onChange={(e) => setProfile({...profile, phone: e.target.value})} disabled={!isEditing} className={inputClasses} /></div>
                </div>
                {isEditing && (
                  <div className="flex gap-3 md:gap-4 mt-6 md:mt-8">
                    <button onClick={() => setIsEditing(false)} className="flex-1 border-2 border-gray-800 py-2.5 md:py-3 rounded-xl font-medium hover:bg-gray-800 transition text-sm">Cancel</button>
                    <button onClick={() => { setIsEditing(false); alert('Profile updated!'); }} className="flex-1 bg-[#CAB276] text-white py-2.5 md:py-3 rounded-xl font-medium hover:bg-[#b39a5e] transition text-sm">Save Changes</button>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === "addresses" && (
              <div className="bg-gray-900 rounded-xl md:rounded-2xl shadow-sm p-5 md:p-8 border border-gray-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                  {addresses.map(addr => (
                    <div key={addr.id} className={`p-4 md:p-5 rounded-xl border-2 ${addr.isDefault ? 'border-[#CAB276] bg-gray-900' : 'border-gray-800'}`}>
                      <div className="flex justify-between items-start mb-2"><span className="font-semibold text-white text-sm md:text-base">{addr.label}</span>{addr.isDefault && <span className="text-[10px] md:text-xs bg-[#CAB276] text-white px-2 py-1 rounded">Default</span>}</div>
                      <p className="text-gray-400 text-xs md:text-sm mb-3">{addr.address}</p>
                      <div className="flex gap-2"><button className="text-xs md:text-sm text-[#CAB276] hover:underline">Edit</button><span className="text-gray-300">|</span><button className="text-xs md:text-sm text-red-500 hover:underline">Delete</button></div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-gray-900 rounded-xl md:rounded-2xl shadow-sm p-5 md:p-8 border border-gray-800">
                ...
                <div className="bg-gray-900 border border-green-800 rounded-xl p-4 md:p-6">
                  <div className="flex items-center gap-3 md:gap-4">
                    ...
                    <div><h3 className="font-semibold text-green-500 text-sm md:text-base">M-Pesa</h3><p className="text-xs md:text-sm text-green-500">Pay securely with your M-Pesa mobile money</p></div>
                  </div>
                </div>
              </div>
            )}
